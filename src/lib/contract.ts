...
export const CONTRACT_ADDRESS = "0xd2BF62cCA0b8330c50ee87342C4aAcc3664E4D43";
...
export async function getSigner() {
  ...
  if (Number(network.chainId) !== CHAIN_ID) {
    throw new Error(
      `Please switch to Polkadot Hub TestNet (Chain ID: ${CHAIN_ID})`
    );
  }
...
export async function makeDeposit() {
  ...
  try {
    ...
    console.log("✅ [Contract] Deposit amount:", ethers.formatEther(depositAmount), "PAS");
    ...
    throw new Error(error instanceof Error ? error.message : "Deposit transaction failed");
  }
}
...
export async function redeemDeposit() {
  ...
  try {
    ...
    console.log("⏰ [Contract] Deadline:", new Date(Number(deadline) * 1000).toISOString());
    console.log("⏰ [Contract] Current time:", new Date(now * 1000).toISOString());
    console.log("⏰ [Contract] Redeeming", isBeforeDeadline ? "BEFORE" : "AFTER", "deadline");
    ...
    throw new Error(error instanceof Error ? error.message : "Redeem transaction failed");
  }
}
